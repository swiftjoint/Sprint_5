...
from data import generate_random_name, generate_random_email, generate_random_password
...
class TestRegistrationAndLogin:
    ...
    def test_button_login_from_personal_cabinet(self, driver, login_lk):
        WebDriverWait(driver, 10).until(
            expected_conditions.visibility_of_element_located
            (TestAutorisation.AUTHORIZED_USER))
        element = driver.find_element(*TestAutorisation.AUTHORIZED_USER)
    ...
    def test_login_via_registration_form_button(self, driver, log_in_via_reg_btn):
        WebDriverWait(driver, 10).until(
            expected_conditions.visibility_of_element_located
            (TestAutorisation.AUTHORIZED_USER))
        element = driver.find_element(*TestAutorisation.AUTHORIZED_USER)
    ...
    def test_login_via_reset_password_form_button(self, driver, log_in_via_reset_btn):
        WebDriverWait(driver, 10).until(
            expected_conditions.visibility_of_element_located
            (TestAutorisation.AUTHORIZED_USER))
        element = driver.find_element(*TestAutorisation.AUTHORIZED_USER)
